...
class IntentRouter {
    constructor(_fileSystemService, _openaiService) {
...
Respond with just the intent category.`;
        try {
            const response = await this._openaiService.generateResponse([{ role: 'user', content: intentPrompt }], 'You are an intent classifier for a software development assistant.');
    ...
    async _generateResponse(userMessage, intent) {
        ...
        const response = await this._openaiService.generateResponse(this._conversationHistory, systemPrompt);
    ...
    _extractActions(responseContent, intent) {
        const actions = [];
            ...
            switch (action.type) {
                ...
                case 'file_write':
                    if (action.parameters?.filePath && action.parameters?.content) {
                        await this._fileSystemService.writeFile(action.parameters.filePath, action.parameters.content);
                        return true;
                    }
                    break;
                case 'file_create':
                    if (action.parameters?.filePath && action.parameters?.content) {
                        await this._fileSystemService.createFile(action.parameters.filePath, action.parameters.content);
                        return true;
                    }
                    break;
                default:
