...
const terminalScriptHelpers_1 = require("./terminalScriptHelpers");
(0, terminalScriptHelpers_1.checkWindows)();
...
let zshBuiltinsCommandDescriptionsCache = new Map();
async function createCommandDescriptionsCache() {
    const cachedCommandDescriptions = new Map();
    let output = '';
    const zshVersionOutput = await (0, terminalScriptHelpers_1.execAsync)('zsh --version').then(r => r.stdout);
    ...
    try {
        output = await (0, terminalScriptHelpers_1.execAsync)('pandoc --from man --to markdown --wrap=none < $(man -w zshbuiltins)').then(r => r.stdout);
    }
    catch {
    }
    ...
    for (const [command, lines] of commands) {
        ...
        const args = (await (0, terminalScriptHelpers_1.execAsync)(`pandoc --from markdown --to plain <<< "${formattedArgs}"`)).stdout.trim();
...
function formatLineAsMarkdown(text) {
...
main();
//# sourceMappingURL=pullZshBuiltins.js.map