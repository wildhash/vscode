...
let fishBuiltinsCommandDescriptionsCache = new Map();
...
async function createCommandDescriptionsCache() {
    ...
    try {
        ...
        const builtinsOutput = await (0, terminalScriptHelpers_1.execAsync)('fish -c "builtin -n"').then(r => r.stdout.trim());
        ...
        for (const cmd of builtins) {
            try {
                ...
                const helpOutput = await (0, terminalScriptHelpers_1.execAsync)(`fish -c "${cmd} --help 2>&1"`).then(r => r.stdout);
                let set = false;
                if (helpOutput && !helpOutput.includes('No help for function') && !helpOutput.includes('See the web documentation')) {
                    const cleanHelpText = (0, terminalScriptHelpers_1.cleanupText)(helpOutput);
 ...
 */
function extractHelpContent(cmd, lines) {
...
main();
//# sourceMappingURL=pullFishBuiltins.js.map