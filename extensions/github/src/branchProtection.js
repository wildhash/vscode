...
class GitHubBranchProtectionProviderManager {
    ...
    constructor(gitAPI, globalState, octokitService, logger, telemetryReporter) {
        ...
        this.disposables = new util_js_1.DisposableStore();
        this.providerDisposables = new util_js_1.DisposableStore();
        this._enabled = false;
    ...
    async updateRepositoryBranchProtection() {
        ...
        try {
            for (const remote of this.repository.state.remotes) {
                const repository = (0, util_js_1.getRepositoryFromUrl)(remote.pushUrl ?? remote.fetchUrl ?? '');
                if (!repository) {
                    continue;
                }
                // Repository details
