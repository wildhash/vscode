...
class GithubRemoteSourceProvider {
    ...
    async getRemoteSources(query) {
        const octokit = await (0, auth_js_1.getOctokit)();
        if (query) {
            const repository = (0, util_js_1.getRepositoryFromUrl)(query);
            if (repository) {
    ...
    async getQueryRemoteSources(octokit, query) {
        ...
        const repository = (0, util_js_1.getRepositoryFromQuery)(query);
        if (repository) {
    ...
    async getBranches(url) {
        const repository = (0, util_js_1.getRepositoryFromUrl)(url);
        if (!repository) {
            return [];
        }
        const octokit = await (0, auth_js_1.getOctokit)();
    ...
    async getRemoteSourceActions(url) {
        ...
        return [{
                ...
                icon: 'github',
                run(branch) {
                    const link = (0, links_js_1.getBranchLink)(url, branch);
                ...
                icon: 'globe',
                run(branch) {
                    const link = (0, links_js_1.getBranchLink)(url, branch, (0, links_js_1.getVscodeDevHost)());
                    vscode_1.env.openExternal(vscode_1.Uri.parse(link));
                }
            }];
    }
